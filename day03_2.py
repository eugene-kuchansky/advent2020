...
def move_x(n, x, y, step):
    return [(x + step * i, y) for i in range(1, n + 1)]


def move_y(n, x, y, step):
    return [(x, y + step * i) for i in range(1, n + 1)]
...
def build(moves):
    ...
    for move in moves:
        #print(move)
        dir_, steps = get_dir(move)
        if dir_ == "R":
            new_positions = move_x(steps, x, y, 1)
        elif dir_ == "L":
            new_positions = move_x(steps, x, y, -1)
        elif dir_ == "U":
            new_positions = move_y(steps, x, y, 1)
        elif dir_ == "D":
            new_positions = move_y(steps, x, y, -1)
        ...
        for i, position in enumerate(new_positions):
            if position not in position_to_steps:
                position_to_steps[position] = curr_steps + i + 1
        curr_steps += steps
...
def calc():
    ...
    position_to_steps1 = build(moves1)
    position_to_steps2 = build(moves2)
    
    positions1 = set(position_to_steps1.keys())
    positions2 = set(position_to_steps2.keys())
    
    common_positions = positions1.intersection(positions2)
    min_steps = min(position_to_steps1[position] + position_to_steps2[position] for position in common_positions)
    print(min_steps)
